static char help[] =
    ...
    "https://link.springer.com/article/10.1007/s42967-023-00297-3\n\n\n";
...
typedef enum _fieldidx { C_FIELD_ID = 0, P_FIELD_ID, NUM_FIELDS } FieldIdx;
...
static void C_0(PetscInt dim, PetscInt Nf, PetscInt NfAux,
  ...
  const PetscScalar gradp[] = {u_x[uOff_x[P_FIELD_ID]],
                               u_x[uOff_x[P_FIELD_ID] + 1]};
  const PetscScalar crossp[] = {gradp[0] * gradp[0], gradp[0] * gradp[1],
                                gradp[1] * gradp[1]};
  ...
  for (PetscInt k = 0; k < 3; k++)
    f0[k] = u_t[uOff[C_FIELD_ID] + k] - c2 * crossp[k] +
            alpha * fnorm * u[uOff[C_FIELD_ID] + k];
}

/* Jacobian for C against C basis functions */
static void JC_0_c0c0(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                      const PetscInt uOff[], const PetscInt uOff_x[],
                      const PetscScalar u[], const PetscScalar u_t[],
                      const PetscScalar u_x[], const PetscInt aOff[],
                      const PetscInt aOff_x[], const PetscScalar a[],
  ...
  for (PetscInt k = 0; k < 3; k++) {
    for (PetscInt j = 0; j < 3; j++)
      J[k * 3 + j] = alpha * dfnorm * dC[j] * u[uOff[C_FIELD_ID] + k];
 ...
 */
static void JC_0_c0p1(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                      const PetscInt uOff[], const PetscInt uOff_x[],
                      const PetscScalar u[], const PetscScalar u_t[],
                      const PetscScalar u_x[], const PetscInt aOff[],
                      const PetscInt aOff_x[], const PetscScalar a[],
...
static void C_1(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                const PetscInt uOff[], const PetscInt uOff_x[],
                const PetscScalar u[], const PetscScalar u_t[],
                const PetscScalar u_x[], const PetscInt aOff[],
                const PetscInt aOff_x[], const PetscScalar a[],
  ...
  for (PetscInt k = 0; k < 3; k++)
    for (PetscInt d = 0; d < 2; d++)
      f1[k * 2 + d] = PetscSqr(D) * u_x[uOff_x[C_FIELD_ID] + k * 2 + d];
}

/* Jacobian for C against gradients of C basis functions */
static void JC_1_c1c1(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                      const PetscInt uOff[], const PetscInt uOff_x[],
                      const PetscScalar u[], const PetscScalar u_t[],
                      const PetscScalar u_x[], const PetscInt aOff[],
                      const PetscInt aOff_x[], const PetscScalar a[],
  ...
  for (PetscInt k = 0; k < 3; k++)
    for (PetscInt d = 0; d < 2; d++)
      J[k * (3 + 1) * 2 * 2 + d * 2 + d] = PetscSqr(D);
}

/* residual for P when tested against basis functions.
   The source term always comes from the auxiliary vec because it needs to have
   zero mean */
static void P_0(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                const PetscInt uOff[], const PetscInt uOff_x[],
                const PetscScalar u[], const PetscScalar u_t[],
                const PetscScalar u_x[], const PetscInt aOff[],
                const PetscInt aOff_x[], const PetscScalar a[],
...
static inline void QuadraticRoots(PetscReal a, PetscReal b, PetscReal c,
                                  PetscReal x[2]);

/* compute shift to make C positive definite */
static inline PetscReal FIX_C(PetscScalar C00, PetscScalar C01,
                              PetscScalar C11) {
...
static void P_1(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                const PetscInt uOff[], const PetscInt uOff_x[],
                const PetscScalar u[], const PetscScalar u_t[],
                const PetscScalar u_x[], const PetscInt aOff[],
                const PetscInt aOff_x[], const PetscScalar a[],
...
/* Same as above for the P-only subproblem for initial conditions: the
 * conductivity values come from the auxiliary vec */
static void P_1_aux(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                    const PetscInt uOff[], const PetscInt uOff_x[],
                    const PetscScalar u[], const PetscScalar u_t[],
                    const PetscScalar u_x[], const PetscInt aOff[],
                    const PetscInt aOff_x[], const PetscScalar a[],
...
static void JP_1_p1p1(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                      const PetscInt uOff[], const PetscInt uOff_x[],
                      const PetscScalar u[], const PetscScalar u_t[],
                      const PetscScalar u_x[], const PetscInt aOff[],
                      const PetscInt aOff_x[], const PetscScalar a[],
...
static void JP_1_p1p1_aux(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                          const PetscInt uOff[], const PetscInt uOff_x[],
                          const PetscScalar u[], const PetscScalar u_t[],
                          const PetscScalar u_x[], const PetscInt aOff[],
                          const PetscInt aOff_x[], const PetscScalar a[],
 ...
 */
static void JP_1_p1c0(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                      const PetscInt uOff[], const PetscInt uOff_x[],
                      const PetscScalar u[], const PetscScalar u_t[],
                      const PetscScalar u_x[], const PetscInt aOff[],
                      const PetscInt aOff_x[], const PetscScalar a[],
...
static PetscErrorCode source_0(PetscInt dim, PetscReal time,
...
static PetscErrorCode source_1(PetscInt dim, PetscReal time,
                               const PetscReal x[], PetscInt Nf, PetscScalar *u,
                               void *ctx) {
  PetscScalar ut[1];
...
static void average(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                    const PetscInt uOff[], const PetscInt uOff_x[],
                    const PetscScalar u[], const PetscScalar u_t[],
                    const PetscScalar u_x[], const PetscInt aOff[],
                    const PetscInt aOff_x[], const PetscScalar a[],
...
static inline void QuadraticRoots(PetscReal a, PetscReal b, PetscReal c,
                                  PetscReal x[2]) {
  PetscReal delta =
      PetscMax(b * b - 4 * a * c, 0); /* eigenvalues symmetric matrix */
  PetscReal temp =
      -0.5 * (b + PetscCopysignReal(1.0, b) * PetscSqrtReal(delta));
...
static void energy(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                   const PetscInt uOff[], const PetscInt uOff_x[],
                   const PetscScalar u[], const PetscScalar u_t[],
                   const PetscScalar u_x[], const PetscInt aOff[],
                   const PetscInt aOff_x[], const PetscScalar a[],
  ...
  const PetscScalar gradp[] = {u_x[uOff_x[P_FIELD_ID]],
                               u_x[uOff_x[P_FIELD_ID] + 1]};
  const PetscScalar gradC00[] = {u_x[uOff_x[C_FIELD_ID] + 0],
                                 u_x[uOff_x[C_FIELD_ID] + 1]};
  const PetscScalar gradC01[] = {u_x[uOff_x[C_FIELD_ID] + 2],
                                 u_x[uOff_x[C_FIELD_ID] + 3]};
  const PetscScalar gradC11[] = {u_x[uOff_x[C_FIELD_ID] + 4],
                                 u_x[uOff_x[C_FIELD_ID] + 5]};
  const PetscScalar normC = NORM2C(C00, C01, C11);
  const PetscScalar normgradC = NORM2C(gradC00[0], gradC01[0], gradC11[0]) +
                                NORM2C(gradC00[1], gradC01[1], gradC11[1]);
  const PetscScalar nexp = gamma / 2.0;

  const PetscScalar t0 = PetscSqr(D) / 2.0 * normgradC;
  const PetscScalar t1 =
      c2 * (gradp[0] * ((C00 + r) * gradp[0] + C01 * gradp[1]) +
            gradp[1] * (C10 * gradp[0] + (C11 + r) * gradp[1]));
  const PetscScalar t2 = alpha / gamma * PetscPowScalar(normC, nexp);

  obj[0] = t0 + t1 + t2;
}

/* functionals to be integrated: ellipticity_fail -> 0 means C+r is elliptic at
 * quadrature point, otherwise it returns the absolute value of the most
 * negative eigenvalue */
static void ellipticity_fail(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                             const PetscInt uOff[], const PetscInt uOff_x[],
                             const PetscScalar u[], const PetscScalar u_t[],
                             const PetscScalar u_x[], const PetscInt aOff[],
                             const PetscInt aOff_x[], const PetscScalar a[],
  ...
  PetscReal eigs[2];
  QuadraticRoots(1, -(C00 + C11), C00 * C11 - PetscSqr(C01), eigs);
  if (eigs[0] < 0 || eigs[1] < 0)
    obj[0] = -PetscMin(eigs[0], eigs[1]);
  else
    obj[0] = 0.0;
}

/* initial conditions for C: eq. 16 */
static PetscErrorCode initial_conditions_C_0(PetscInt dim, PetscReal time,
                                             const PetscReal xx[], PetscInt Nc,
...
static PetscErrorCode initial_conditions_C_1(PetscInt dim, PetscReal time,
                                             const PetscReal xx[], PetscInt Nc,
                                             PetscScalar *u, void *ctx) {
...
static PetscErrorCode initial_conditions_C_2(PetscInt dim, PetscReal time,
                                             const PetscReal xx[], PetscInt Nc,
                                             PetscScalar *u, void *ctx) {
  u[0] = 0;
  u[1] = 0;
...
static void flux(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                 const PetscInt uOff[], const PetscInt uOff_x[],
                 const PetscScalar u[], const PetscScalar u_t[],
                 const PetscScalar u_x[], const PetscInt aOff[],
                 const PetscInt aOff_x[], const PetscScalar a[],
...
static void normc(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                  const PetscInt uOff[], const PetscInt uOff_x[],
                  const PetscScalar u[], const PetscScalar u_t[],
                  const PetscScalar u_x[], const PetscInt aOff[],
                  const PetscInt aOff_x[], const PetscScalar a[],
...
static void zero(PetscInt dim, PetscInt Nf, PetscInt NfAux,
                 const PetscInt uOff[], const PetscInt uOff_x[],
                 const PetscScalar u[], const PetscScalar u_t[],
                 const PetscScalar u_x[], const PetscInt aOff[],
                 const PetscInt aOff_x[], const PetscScalar a[],
...
static PetscErrorCode zerof(PetscInt dim, PetscReal time, const PetscReal xx[],
...
static PetscErrorCode constantf(PetscInt dim, PetscReal time,
                                const PetscReal xx[], PetscInt Nc,
                                PetscScalar *u, void *ctx) {
  PetscInt d;
...
static PetscErrorCode ProcessOptions(AppCtx *options) {
  ...
  PetscCall(PetscOptionsReal("-alpha", "alpha", __FILE__, options->alpha,
                             &options->alpha, NULL));
  PetscCall(PetscOptionsReal("-gamma", "gamma", __FILE__, options->gamma,
                             &options->gamma, NULL));
  PetscCall(
      PetscOptionsReal("-c", "c", __FILE__, options->c, &options->c, NULL));
  PetscCall(
      PetscOptionsReal("-d", "D", __FILE__, options->D, &options->D, NULL));
  PetscCall(PetscOptionsReal("-eps", "eps", __FILE__, options->eps,
                             &options->eps, NULL));
  PetscCall(
      PetscOptionsReal("-r", "r", __FILE__, options->r, &options->r, NULL));
  PetscCall(
      PetscOptionsRealArray("-x0", "x0", __FILE__, options->x0, &dim, NULL));
  PetscCall(PetscOptionsInt("-ic_num", "ic_num", __FILE__, options->ic_num,
                            &options->ic_num, NULL));
  PetscCall(PetscOptionsInt("-source_num", "source_num", __FILE__,
                            options->source_num, &options->source_num, NULL));
  PetscCall(PetscOptionsBool("-lump", "use mass lumping", __FILE__,
                             options->lump, &options->lump, NULL));
  PetscCall(PetscOptionsInt(
      "-fix_c", "shift conductivity to always be positive semi-definite",
      __FILE__, options->fix_c, &options->fix_c, NULL));
  PetscCall(PetscOptionsBool("-amr", "use adaptive mesh refinement", __FILE__,
                             options->amr, &options->amr, NULL));
  PetscCall(PetscOptionsBool("-test_restart", "test restarting files", __FILE__,
                             options->test_restart, &options->test_restart,
                             NULL));
  if (!options->test_restart) {
    PetscCall(PetscOptionsString(
        "-load", "filename with data to be loaded for restarting", __FILE__,
        options->load_filename, options->load_filename, PETSC_MAX_PATH_LEN,
        &options->load));
    PetscCall(PetscOptionsString(
        "-save", "filename with data to be saved for restarting", __FILE__,
        options->save_filename, options->save_filename, PETSC_MAX_PATH_LEN,
        &options->save));
    if (options->save)
      PetscCall(PetscOptionsInt(
          "-save_every", "save every n timestep (-1 saves only the last)",
          __FILE__, options->save_every, &options->save_every, NULL));
  }
  PetscCall(PetscOptionsBool(
      "-monitor_ellipticity", "Dump locations of ellipticity violation",
      __FILE__, options->ellipticity, &options->ellipticity, NULL));
  PetscOptionsEnd();
...
static PetscErrorCode SaveToFile(DM dm, Vec u, const char *filename) {
  ...
  PetscCall(PetscOptionsInsertString(
      NULL, "-dm_plex_view_hdf5_storage_version 3.0.0"));
  PetscCall(PetscViewerHDF5Open(PetscObjectComm((PetscObject)dm), filename,
                                FILE_MODE_WRITE, &viewer));
  PetscCall(PetscViewerHDF5WriteAttribute(viewer, NULL, "numlevels", PETSC_INT,
                                          &numlevels));
  ...
  for (PetscInt level = numlevels - 1; level >= 0; level--) {
    ...
    PetscCall(PetscSNPrintf(groupname, sizeof(groupname),
                            "level_%" PetscInt_FMT, level));
    ...
    PetscCall(PetscViewerHDF5WriteAttribute(viewer, NULL, "meshname",
                                            PETSC_STRING, dmname));
    PetscCall(PetscViewerHDF5WriteAttribute(viewer, NULL, "refinelevel",
                                            PETSC_INT, &rr));
    PetscCall(PetscViewerHDF5WriteAttribute(viewer, NULL, "coarsenlevel",
                                            PETSC_INT, &cc));
    PetscCall(PetscViewerHDF5WriteAttribute(viewer, NULL, "refRegular",
                                            PETSC_BOOL, &isRegular));
    PetscCall(PetscViewerHDF5WriteAttribute(viewer, NULL, "refUniform",
                                            PETSC_BOOL, &isUniform));
    ...
    if (level) {
      ...
      for (PetscInt c = cStart; c < cEnd; c++)
        PetscCall(PetscSectionSetDof(section, c, 1));
      ...
      for (PetscInt c = ccStart; c < ccEnd; c++) {
        ...
        DMPolytopeType *rct;
        PetscInt gnum = gidx[c - cpStart] >= 0 ? gidx[c - cpStart]
                                               : -(gidx[c - cpStart] + 1);

        PetscCall(DMPlexGetCellType(cdm, c, &ct));
        PetscCall(DMPlexTransformCellTransform(tr, ct, c, NULL, &Nt, &rct,
                                               &rsize, &rcone, &rornt));
        for (PetscInt r = 0; r < rsize[Nt - 1]; ++r) {
          PetscInt pNew;

          PetscCall(
              DMPlexTransformGetTargetPoint(tr, ct, rct[Nt - 1], c, r, &pNew));
  ...
  SETERRQ(PetscObjectComm((PetscObject)dm), PETSC_ERR_SUP,
          "Needs HDF5 support. Please reconfigure using --download-hdf5");
#endif
}

static PetscErrorCode LoadFromFile(MPI_Comm comm, const char *filename,
                                   DM *odm) {
#if defined(PETSC_HAVE_HDF5)
  PetscViewerFormat format = PETSC_VIEWER_HDF5_PETSC;
  PetscViewer viewer;
  ...
  PetscCall(PetscViewerHDF5ReadAttribute(viewer, NULL, "numlevels", PETSC_INT,
                                         NULL, &numlevels));
  PetscCall(PetscViewerPushFormat(viewer, format));
  for (PetscInt level = 0; level < numlevels; level++) {
    ...
    PetscCall(PetscSNPrintf(groupname, sizeof(groupname),
                            "level_%" PetscInt_FMT, level));
    PetscCall(PetscViewerHDF5PushGroup(viewer, groupname));
    PetscCall(PetscViewerHDF5ReadAttribute(viewer, NULL, "meshname",
                                           PETSC_STRING, NULL, &dmname));
    PetscCall(PetscViewerHDF5ReadAttribute(viewer, NULL, "refinelevel",
                                           PETSC_INT, NULL, &rr));
    PetscCall(PetscViewerHDF5ReadAttribute(viewer, NULL, "coarsenlevel",
                                           PETSC_INT, NULL, &cc));
    PetscCall(PetscViewerHDF5ReadAttribute(viewer, NULL, "refRegular",
                                           PETSC_BOOL, NULL, &isRegular));
    PetscCall(PetscViewerHDF5ReadAttribute(viewer, NULL, "refUniform",
                                           PETSC_BOOL, NULL, &isUniform));
    ...
    if (!level) { /* partition the coarse level only */
      ...
      PetscLayout clayout;
      PetscScalar *array;
      PetscInt *cranks_leaf, *cranks_root, *npoints, *points, *ranks, *starts,
          *gidxs;
      ...
      PetscCall(PetscMalloc4(cEnd - cStart, &points, cEnd - cStart, &ranks,
                             nparts + 1, &starts, cEnd - cStart, &gidxs));
      ...
      PetscCall(PetscSFReduceBegin(sfXC, MPIU_INT, cranks_leaf, cranks_root,
                                   MPI_REPLACE));
      PetscCall(PetscSFReduceEnd(sfXC, MPIU_INT, cranks_leaf, cranks_root,
                                 MPI_REPLACE));

      PetscCall(VecGetArray(map, &array));
      for (PetscInt c = 0; c < cEnd - cStart; c++)
        gidxs[c] = (PetscInt)PetscRealPart(array[c]);
      PetscCall(VecRestoreArray(map, &array));

      PetscCall(PetscLayoutCreate(comm, &clayout));
      PetscCall(PetscLayoutSetLocalSize(clayout, nr));
      PetscCall(PetscSFSetGraphLayout(sf, clayout, cEnd - cStart, NULL,
                                      PETSC_OWN_POINTER, gidxs));
      PetscCall(PetscLayoutDestroy(&clayout));

      PetscCall(
          PetscSFBcastBegin(sf, MPIU_INT, cranks_root, ranks, MPI_REPLACE));
      ...
      starts[0] = 0;
      for (PetscInt c = 0; c < nparts; c++)
        starts[c + 1] = starts[c] + npoints[c];
      for (PetscInt c = 0; c < cEnd - cStart; c++)
        points[starts[ranks[c]]++] = c;
      PetscCall(PetscPartitionerSetType(part, PETSCPARTITIONERSHELL));
      PetscCall(
          PetscPartitionerShellSetPartition(part, nparts, npoints, points));
...
static PetscErrorCode ProjectSource(DM dm, PetscReal time, AppCtx *ctx) {
  ...
  PetscDS ds;
  PetscErrorCode (*funcs[NUM_FIELDS])(PetscInt, PetscReal, const PetscReal[],
...
static PetscErrorCode CreatePotentialNullSpace(DM dm, PetscInt ofield,
                                               PetscInt nfield,
  ...
  PetscCall(MatNullSpaceCreate(PetscObjectComm((PetscObject)dm), PETSC_FALSE, 1,
                               &vec, nullSpace));
  /* break ref cycles */
...
static PetscErrorCode DMGetLumpedMass(DM dm, PetscBool local,
                                      Vec *lumped_mass) {
  PetscBool has;
  ...
  if (!has) {
    ...
    PetscCall(
        PetscObjectQuery((PetscObject)dm, "IS potential", (PetscObject *)&is));
    if (!is) {
      PetscInt fields[NUM_FIELDS] = {C_FIELD_ID, P_FIELD_ID};

      PetscCall(DMCreateSubDM(dm, NUM_FIELDS - 1, fields + 1, &is, NULL));
      PetscCall(
          PetscObjectCompose((PetscObject)dm, "IS potential", (PetscObject)is));
...
static PetscErrorCode DMRestoreLumpedMass(DM dm, PetscBool local,
                                          Vec *lumped_mass) {
  PetscFunctionBeginUser;
  if (local)
    PetscCall(DMRestoreNamedLocalVector(dm, "lumped_mass", lumped_mass));
  else
    PetscCall(DMRestoreNamedGlobalVector(dm, "lumped_mass", lumped_mass));
  PetscFunctionReturn(PETSC_SUCCESS);
}

/* callbacks for lumped mass matrix residual and Jacobian */
static PetscErrorCode DMPlexTSComputeIFunctionFEM_Lumped(DM dm, PetscReal time,
                                                         Vec locX, Vec locX_t,
                                                         Vec locF, void *user) {
...
static PetscErrorCode DMPlexTSComputeIJacobianFEM_Lumped(DM dm, PetscReal time,
                                                         Vec locX, Vec locX_t,
                                                         PetscReal X_tShift,
                                                         Mat Jac, Mat JacP,
                                                         void *user) {
  ...
  PetscCall(DMPlexTSComputeIJacobianFEM(dm, time, locX, locX_t, 0.0, Jac, JacP,
                                        user));
...
static PetscErrorCode SetupProblem(DM dm, AppCtx *ctx) {
  ...
  PetscCheck(dim == 2 && cdim == 2, PetscObjectComm((PetscObject)dm),
             PETSC_ERR_SUP, "Only for 2D meshes");
  PetscCall(DMGetDS(dm, &ds));
  ...
  PetscCall(PetscDSSetJacobian(ds, C_FIELD_ID, C_FIELD_ID, JC_0_c0c0, NULL,
                               NULL, JC_1_c1c1));
  PetscCall(PetscDSSetJacobian(ds, C_FIELD_ID, P_FIELD_ID, NULL, JC_0_c0p1,
                               NULL, NULL));
  PetscCall(PetscDSSetJacobian(ds, P_FIELD_ID, C_FIELD_ID, NULL, NULL,
                               JP_1_p1c0, NULL));
  PetscCall(PetscDSSetJacobian(ds, P_FIELD_ID, P_FIELD_ID, NULL, NULL, NULL,
                               JP_1_p1p1));

  /* Attach potential nullspace */
  PetscCall(
      DMSetNullSpaceConstructor(dm, P_FIELD_ID, CreatePotentialNullSpace));
  ...
  if (ctx->lump) {
    PetscCall(
        DMTSSetIFunctionLocal(dm, DMPlexTSComputeIFunctionFEM_Lumped, NULL));
    PetscCall(
        DMTSSetIJacobianLocal(dm, DMPlexTSComputeIJacobianFEM_Lumped, NULL));
  } else {
...
static PetscErrorCode SetupDiscretization(DM dm, AppCtx *ctx) {
  ...
  PetscCallMPI(
      MPIU_Allreduce(MPI_IN_PLACE, &simplex, 1, MPIU_BOOL, MPI_LOR, comm));
  PetscCall(DMDestroy(&plex));

  /* We model Cij with Cij = Cji -> dim*(dim+1)/2 components */
  PetscCall(PetscFECreateDefault(comm, dim, (dim * (dim + 1)) / 2, simplex,
                                 "c_", -1, &feC));
  ...
  PetscCall(
      PetscObjectCompose((PetscObject)feP, "nullspace", (PetscObject)nsp));
...
static PetscErrorCode CreateMesh(MPI_Comm comm, DM *dm, AppCtx *ctx) {
  ...
  if (ctx->load) {
    ...
    PetscCall(PetscOptionsFList(
        "-dm_mat_type", "Matrix type used for created matrices", "DMSetMatType",
        MatList, MATAIJ, typeName, sizeof(typeName), &flg));
    if (flg) PetscCall(DMSetMatType(*dm, typeName));
    PetscCall(PetscOptionsBoundedInt("-dm_refine",
                                     "The number of uniform refinements",
                                     "DMCreate", refine, &refine, NULL, 0));
    PetscCall(PetscOptionsBoundedInt(
        "-dm_refine_hierarchy", "The number of uniform refinements", "DMCreate",
        refine, &refine, &isHierarchy, 0));
    PetscOptionsEnd();
    if (refine) {
    ...
    PetscCall(DMLocalizeCoordinates(*dm));
    {
      ...
      PetscBool flg;
      PetscOptionsBegin(comm, "", "Additional mesh options", "DMPLEX");
      PetscCall(PetscOptionsFList("-dm_plex_convert_type",
                                  "Convert DMPlex to another format", __FILE__,
                                  DMList, DMPLEX, convType, 256, &flg));
      PetscOptionsEnd();
      if (flg) {
        DM dmConv;
...
static PetscErrorCode ZeroMeanPotential(DM dm, Vec u) {
  ...
  PetscCall(
      PetscObjectQuery((PetscObject)dm, "IS potential", (PetscObject *)&is));
  PetscCheck(is, PetscObjectComm((PetscObject)dm), PETSC_ERR_PLIB,
             "Missing potential IS");
  PetscCall(DMGetDS(dm, &ds));
  PetscCall(PetscDSSetObjective(ds, P_FIELD_ID, average));
  PetscCall(DMPlexComputeIntegralFEM(dm, u, vals, NULL));
  PetscCall(PetscDSSetObjective(ds, P_FIELD_ID, zero));
...
static PetscErrorCode SetInitialConditionsAndTolerances(TS ts, PetscInt nv,
                                                        Vec vecs[],
                                                        PetscBool valid) {
  DM dm;
  ...
  if (valid) {
    ...
    PetscCall(
        PetscObjectCompose((PetscObject)dm, "IS potential", (PetscObject)isp));
    ...
    for (PetscInt i = 0; i < nv; i++) {
      PetscCall(ZeroMeanPotential(dm, vecs[i]));
    }
    PetscFunctionReturn(PETSC_SUCCESS);
  }
...
static PetscErrorCode ResizeSetUp(TS ts, PetscInt nstep, PetscReal time, Vec u,
                                  PetscBool *resize, void *vctx) {
  ...
  PetscCall(DMDestroy(&plex));
  PetscCall(VecCreateMPI(PetscObjectComm((PetscObject)ts),
                         NUM_FIELDS * (cEnd - cStart), PETSC_DECIDE,
                         &ellVecCellsF));
  PetscCall(VecCreateMPI(PetscObjectComm((PetscObject)ts), cEnd - cStart,
                         PETSC_DECIDE, &ellVecCells));
  ...
  PetscCall(PetscDSSetObjective(ds, C_FIELD_ID, energy));
  PetscCall(
      VecStrideGather(ellVecCellsF, C_FIELD_ID, ellVecCells, INSERT_VALUES));
  ...
  if (norm && !ctx->cnt) {
    ...
    *resize = PETSC_TRUE;
    if (!ctx->refineTag) {
      ...
      PetscCall(
          PetscObjectSetOptionsPrefix((PetscObject)ctx->refineTag, "refine_"));
      ...
      PetscCall(PetscObjectViewFromOptions((PetscObject)ctx->refineTag, NULL,
                                           "-tag_view"));
    }
    PetscCall(DMLabelDestroy(&ctx->adaptLabel));
    PetscCall(DMLabelCreate(PetscObjectComm((PetscObject)ts), "adapt",
                            &ctx->adaptLabel));
...
static PetscErrorCode ResizeTransfer(TS ts, PetscInt nv, Vec vecsin[],
  ...
  PetscFunctionBeginUser;
  PetscCall(TSGetDM(ts, &dm));
  PetscCheck(actx->adaptLabel, PetscObjectComm((PetscObject)ts),
             PETSC_ERR_ARG_WRONGSTATE, "Missing adaptLabel");
  PetscCall(DMAdaptLabel(dm, actx->adaptLabel, &adm));
  PetscCheck(adm, PetscObjectComm((PetscObject)ts), PETSC_ERR_ARG_WRONGSTATE,
             "Missing adapted DM");
  ...
  for (PetscInt i = 0; i < nv; i++) {
    ...
    PetscCall(
        DMForestTransferVec(dm, vecsin[i], adm, vecsout[i], PETSC_TRUE, time));
  }
  ...
  PetscCall(TSGetApplicationContext(ts, &ctx));
  PetscCall(
      DMSetNullSpaceConstructor(adm, P_FIELD_ID, CreatePotentialNullSpace));
...
static PetscErrorCode OutputVTK(DM dm, const char *filename,
                                PetscViewer *viewer) {
  PetscFunctionBeginUser;
...
static PetscErrorCode Monitor(TS ts, PetscInt stepnum, PetscReal time, Vec u,
                              void *vctx) {
  ...
  if (ctx->ellipticity) {
    void (*funcs[NUM_FIELDS])(
        PetscInt dim, PetscInt Nf, PetscInt NfAux, const PetscInt uOff[],
    ...
    PetscCall(PetscSNPrintf(filename, sizeof filename,
                            "ellipticity_fail-%03" PetscInt_FMT ".vtu",
                            stepnum));
  ...
  PetscCall(PetscPrintf(
      ...
      "%4" PetscInt_FMT " TS: time %g, energy %g, intp %g, ell %g\n", stepnum,
      (double)time, (double)PetscRealPart(vals[C_FIELD_ID]),
      (double)PetscRealPart(vals[P_FIELD_ID]),
      (double)PetscRealPart(vals[NUM_FIELDS + C_FIELD_ID])));
  PetscFunctionReturn(PETSC_SUCCESS);
}

/* Save restart information */
static PetscErrorCode MonitorSave(TS ts, PetscInt steps, PetscReal time, Vec u,
                                  void *vctx) {
  DM dm;
  ...
  if ((save_every > 0 && steps % save_every == 0) ||
      (save_every == -1 && reason) || save_every < -1)
    PetscCall(SaveToFile(dm, u, ctx->save_filename));
  PetscFunctionReturn(PETSC_SUCCESS);
}

/* Make potential zero mean after SNES solve */
static PetscErrorCode PostStage(TS ts, PetscReal stagetime, PetscInt stageindex,
                                Vec *Y) {
  DM dm;
  Vec u = Y[stageindex];
  SNES snes;
  ...
  PetscCall(PetscPrintf(PetscObjectComm((PetscObject)ts),
                        "         step %" PetscInt_FMT " stage %" PetscInt_FMT
                        " nonlinear its %" PetscInt_FMT
                        ", linear its %" PetscInt_FMT "\n",
...
static PetscErrorCode VecViewFlux(Vec u, const char *opts) {
  ...
  PetscCall(DMPlexIsSimplex(plex, &simplex));
  PetscCall(DMDestroy(&plex));
  PetscCall(PetscFECreateDefault(PetscObjectComm((PetscObject)dm), dim, dim,
                                 simplex, "flux_", -1, &feFluxC));
  PetscCall(PetscFECreateDefault(PetscObjectComm((PetscObject)dm), dim, 1,
                                 simplex, "normc_", -1, &feNormC));
...
static PetscErrorCode Run(MPI_Comm comm, AppCtx *ctx) {
  ...
  if (ctx->test_restart) {
    ...
    PetscCall(
        PetscViewerGetSubViewer(PETSC_VIEWER_STDOUT_WORLD, comm, &subviewer));
    if (ctx->load)
      PetscCall(PetscViewerASCIIPrintf(subviewer, "rank %d loading from %s\n",
                                       rank, ctx->load_filename));
    if (ctx->save)
      PetscCall(PetscViewerASCIIPrintf(subviewer, "rank %d saving to %s\n",
                                       rank, ctx->save_filename));
    PetscCall(PetscViewerRestoreSubViewer(PETSC_VIEWER_STDOUT_WORLD, comm,
                                          &subviewer));
    ...
    if (ctx->load)
      PetscCall(PetscPrintf(comm, "  load : %s\n", ctx->load_filename));
    else
      PetscCall(PetscPrintf(comm, "  IC   : %" PetscInt_FMT "\n", ctx->ic_num));
    PetscCall(
        PetscPrintf(comm, "  S    : %" PetscInt_FMT "\n", ctx->source_num));
  ...
  if (ctx->amr)
    PetscCall(TSSetResize(ts, PETSC_TRUE, ResizeSetUp, ResizeTransfer, actx));
...
int main(int argc, char **argv) {
  ...
  if (ctx.test_restart) { /* Test sequences of save and loads */
    ...
    PetscCall(PetscSNPrintf(ctx.save_filename, sizeof(ctx.save_filename),
                            "test_ex30_seq_%d.h5", rank));
    ...
    PetscCall(PetscSNPrintf(ctx.load_filename, sizeof(ctx.load_filename),
                            "test_ex30_seq_0.h5"));
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                          "Test sequential load from sequential save\n"));
    PetscCall(Run(PETSC_COMM_SELF, &ctx));
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                          "Test parallel load from sequential save\n"));
    PetscCall(Run(PETSC_COMM_WORLD, &ctx));
    /* sequential and parallel runs from parallel save */
    PetscCall(PetscSNPrintf(ctx.load_filename, sizeof(ctx.load_filename),
                            "test_ex30_par.h5"));
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                          "Test sequential load from parallel save\n"));
    PetscCall(Run(PETSC_COMM_SELF, &ctx));
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                          "Test parallel load from parallel save\n"));
    PetscCall(Run(PETSC_COMM_WORLD, &ctx));
...
/*TEST
  ...
  testset:
    args: -dm_plex_box_faces 3,3 -ksp_type preonly -pc_type svd
-c_petscspace_degree 1 -p_petscspace_degree 1 -ts_max_steps 1
-initial_snes_test_jacobian -snes_test_jacobian -initial_snes_type ksponly
-snes_type ksponly -petscpartitioner_type simple -dm_plex_simplex 0
-ts_adapt_type none
    ...
    test:
      suffix: 0_dirk_mg
      nsize: {{1 2}}
      args: -dm_refine_hierarchy 1 -ts_type dirk -pc_type mg -mg_levels_pc_type
bjacobi -mg_levels_sub_pc_factor_levels 2
-mg_levels_sub_pc_factor_mat_ordering_type rcm
-mg_levels_sub_pc_factor_reuse_ordering -mg_coarse_pc_type svd -lump {{0 1}}

    test:
      suffix: 0_dirk_fieldsplit
      nsize: {{1 2}}
      args: -dm_refine 1 -ts_type dirk -pc_type fieldsplit -pc_fieldsplit_type
multiplicative -lump {{0 1}}
...
-dm_plex_convert_type p4est -lump 0

    test:
      requires: p4est
      suffix: 0_p4est_mg
      nsize: {{1 2}}
      args: -dm_forest_minimum_refinement 0 -dm_forest_initial_refinement 2
-dm_plex_convert_type p4est -pc_type mg -mg_coarse_pc_type svd
-mg_levels_pc_type svd -lump 0

  testset:
    requires: hdf5
    args: -test_restart -dm_plex_box_faces 3,3 -ksp_type preonly -pc_type mg
-mg_levels_pc_type svd -c_petscspace_degree 1 -p_petscspace_degree 1
-petscpartitioner_type simple -test_restart
