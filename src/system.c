...
PetscErrorCode formkappa(DM dm, Vec x, PetscInt penal) {
  ...
  PetscCall(
      DMDAGetCorners(dm, &startx, &starty, &startz, &nx, &ny, &nz));
  PetscCall(DMDAVecGetArrayRead(dm, x, &arrayx));
  for (i = 0; i < DIM; ++i) {
    PetscCall(DMDAVecGetArray(dm, kappa[i], &arraykappa[i]));
  }
  ...
  PetscCall(DMDAVecRestoreArrayRead(dm, x, &arrayx));
  for (i = 0; i < DIM; ++i) {
    PetscCall(DMDAVecRestoreArray(dm, kappa[i], &arraykappa[i]));
  }
